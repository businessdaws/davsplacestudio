...
import { Hash, Send, Loader2, Sparkles, ChevronRight, Share2, Info } from 'lucide-react';
import { generateThreads } from '../services/geminiService';
...
export default function ThreadsGenerator({ onQuotaExceeded }: { onQuotaExceeded?: (reason?: string) => void }) {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await generateThreads(topic);
      setResult(data);
    } catch (err: any) {
      console.error(err);
      if (err.status === 429) {
      ...
      setError(err.message || 'Terjadi kesalahan saat membuat thread.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-12 pb-12">
      <section className="text-center space-y-4">
        ...
        )}>Generator Threads</h1>
        <p className={cn(
          "max-w-xl mx-auto font-medium",
          theme === 'dark' ? "text-white/40" : "text-slate-500"
        )}>Seri Twitter dan Threads dengan retensi tinggi untuk keterlibatan maksimal.</p>
      </section>
      ...
      <form onSubmit={handleGenerate} className={cn(
        ...
        <div className="space-y-4">
          ...
          )}>Topik untuk thread</label>
          <textarea
            value={topic}
            onChange={(e) => setTopic(e.target.value)}
            placeholder="Contoh: 10 pelajaran dari 5 tahun bekerja jarak jauh..."
            className={cn(
        ...
        >
          {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Buat Thread <Sparkles className="w-4 h-4" /></>}
        </button>
      </form>
        ...
        {result && (
          <div className="space-y-6">
            <div className="flex items-center justify-between px-2">
               <h3 className={cn(
                 "text-xs font-black uppercase tracking-[0.3em] flex items-center gap-2",
                 theme === 'dark' ? "text-white/30" : "text-slate-400"
               )}>
                 <Share2 className="w-4 h-4" /> Urutan Thread
               </h3>
               <span className={cn(
                 "text-[10px] font-bold uppercase",
                 theme === 'dark' ? "text-white/20" : "text-slate-300"
               )}>{result.length} Tweet</span>
            </div>
            <div className={cn(
              "space-y-6 relative before:absolute before:left-8 before:top-8 before:bottom-8 before:w-px",
              theme === 'dark' ? "before:bg-white/5" : "before:bg-slate-100"
            )}>
              {result.map((text, i) => (
                <motion.div
                  key={i}
                ...
                >
                  <div className={cn(
                    "absolute left-6 top-3 w-4 h-4 rounded-full z-10 border-2",
                    theme === 'dark' ? "bg-[#0F0F0F] border-yellow-400" : "bg-white border-yellow-500 shadow-sm shadow-yellow-500/20"
                  )} />
                  <OutputCard 
                    title={`Tweet ${i + 1}`} 
                    content={text} 
                    isPremium={isPremium}
                    onSave={() => handleSaveAction(`Thread ${i + 1}: ${topic.substring(0, 20)}`, text)}
                  />
                </motion.div>
              ))}
            </div>
